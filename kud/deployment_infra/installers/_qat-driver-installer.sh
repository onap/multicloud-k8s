...
_rename_ssl_conf_section() {
    ...
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "dh895xcc_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/dh895xcc_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "c6xx_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/c6xx_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "d15xx_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/d15xx_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "c3xxx_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/c3xxx_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "200xx_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/200xx_dev${dev}.conf"
    done

    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "dh895xccvf_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/dh895xccvf_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "c6xxvf_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/c6xxvf_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "d15xxvf_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/d15xxvf_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "c3xxxvf_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
	sed -i "s/\[SSL\]/\[SSL${dev}\]/g" "${ROOT_MOUNT_DIR}/etc/c3xxxvf_dev${dev}.conf"
    done
    for file in $(find "${ROOT_MOUNT_DIR}/etc" -type f -name "200xxvf_dev*.conf"); do
	dev=$(echo $file | cut -d '_' -f 2 | tr -cd '[[:digit:]]')
