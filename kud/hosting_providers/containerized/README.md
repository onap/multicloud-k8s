...
$  docker build  --rm -t onap.org/multicloud/kud:latest . -f build/Dockerfile
```
      ...
      containers:
        - name: kud
          ...
          - name: secret-volume
            mountPath: "/.ssh"
          command: ["/bin/sh","-c"]
          args: ["cp -r /.ssh /root/; chmod -R 600 /root/.ssh; ./installer --cluster $CLUSTER-NAME"]
          securityContext:
            privileged: true
      volumes:
      - name: multi-cluster
