...
function install_kubespray {
    ...
    apt-get install -y sshpass make unzip # install make to run mitogen target and unzip is mitogen playbook dependency
    _install_ansible
...
function install_k8s {
	...
	ansible-playbook $verbose -i $kud_inventory $dest_folder/kubespray-$version/cluster.yml -e cluster_name=$cluster_name --become --become-user=root | tee $cluster_log/setup-kubernetes.log

    # Configure environment
    ls -lt $kud_inventory_folder
    ...
    mv -f $kud_inventory_folder/artifacts /opt/kud/multi-cluster/$cluster_name/
}
...
function install_addons {
    ...
    _install_ansible
    ansible-galaxy install $verbose -r $kud_infra_folder/galaxy-requirements.yml --ignore-errors

    ansible-playbook $verbose -i $kud_inventory $kud_playbooks/configure-kud.yml | tee $cluster_log/setup-kud.log
    for addon in ${KUD_ADDONS:-virtlet ovn4nfv nfd}; do
        echo "Deploying $addon using configure-$addon.yml playbook.."
        ansible-playbook $verbose -i $kud_inventory $kud_playbooks/configure-${addon}.yml | tee $cluster_log/setup-${addon}.log
