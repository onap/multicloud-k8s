...
function install_k8s {
    ...
    if [ "$container_runtime" == "docker" ]; then
        ...
        echo "Containerd will be used as the container runtime interface and Kata will be installed"
        ansible-playbook $verbose -i \
             $kud_inventory $dest_folder/kubespray-$version/cluster.yml \
...
function install_addons {
    ...
    if [ "$container_runtime" == "docker" ]; then
        ...
        kud_addons=${KUD_ADDONS:-topology-manager ovn4nfv nfd}
    fi
...
kud_kata_override_variables="container_manager=containerd -e etcd_deployment_type=host -e kubelet_cgroup_driver=cgroupfs"
