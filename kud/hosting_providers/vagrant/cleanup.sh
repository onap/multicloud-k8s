...
(docker rm -vf $(docker ps -a --format "{{.Image}} {{.Names}} {{.ID}}" | awk -v matchExp="$2" '$0 ~ matchExp {print $2}') 2>/dev/null || echo "No running containers using the \"$2\" image, continuing purge.") &&\
  # Remove all images matching arg given after "--purge"`
docker rmi $(docker images --format "{{.Repository}}: {{.ID}}" | grep "$matchExp" | awk -v matchExp="$2" '$0 ~ matchExp {print $2}') 2>/dev/null
else
  # This alternate only removes "stopped" containers
docker rm -vf $(docker ps -a | grep "Exited" | awk '{print $2}') 2>/dev/null || echo "No stopped containers to remove."
fi
