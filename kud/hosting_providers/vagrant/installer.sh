...
function install_addons {
    ...
    for addon in ${KUD_ADDONS:-virtlet ovn4nfv nfd sriov qat optane cmk topology-manager}; do
    ...
    if [[ "${testing_enabled}" == "true" ]]; then
        for addon in ${KUD_ADDONS:-multus virtlet ovn4nfv nfd sriov qat optane cmk topology-manager}; do
            pushd $kud_tests
            bash ${addon}.sh
            popd
        done
