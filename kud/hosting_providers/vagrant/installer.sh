...
function install_k8s {
    ...
    if [ "$container_runtime" == "docker" ]; then
        ...
        /bin/echo -e "\n\e[1;42mContainerd will be used as the container runtime interface and Kata will be installed\e[0m"
        # Because the kud_kata_override_variable has its own quotations in it an
        # eval command is needed to properly execute the ansible script
        ansible_kubespray_cmd="ansible-playbook $verbose -i $kud_inventory $dest_folder/kubespray-$version/cluster.yml -e ${kud_kata_override_variables} --become --become-user=root | sudo tee $log_folder/setup-kubernetes.log"
...
function install_addons {
    ...
    if [ "$container_runtime" == "docker" ]; then
        kud_addons=${KUD_ADDONS:-topology-manager virtlet ovn4nfv nfd sriov qat optane cmk}
    elif [ "$container_runtime" == "containerd" ]; then
        kud_addons=${KUD_ADDONS:-topology-manager ovn4nfv nfd sriov}
    fi
...
kud_kata_override_variables="container_manager=containerd -e etcd_deployment_type=host -e kubelet_cgroup_driver=cgroupfs -e \"{'download_localhost': false}\" -e \"{'download_run_once': false}\""
