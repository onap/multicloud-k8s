...
function get_ovn_central_address {
    ...
    #Check if OVN_CENTRAL_INTERFACE wasn't enslaved under ovs bridge
    local master_interface
    if master_interface="$(sudo ovs-vsctl iface-to-br "${OVN_CENTRAL_INTERFACE}" 2>/dev/null)"; then
        echo "OVN Central Interface has been enslaved by ${master_interface}" >&2
        OVN_CENTRAL_INTERFACE="${master_interface}"
    fi
    ...
    if [[ $ansible_ifconfig != *CHANGED* ]]; then
        echo "Fail to get the OVN central IP address from ${OVN_CENTRAL_INTERFACE} nic" >&2
        exit 1
    fi
