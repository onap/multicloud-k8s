...
sdwan_pod_wan_ip=$(kubectl exec -it $sdwan_pod_name ifconfig $wan_interface  | awk '/inet/{print $2}' | cut -f2 -d ":" | awk 'NR==1 {print $1}')
...
sdwan_pod_wan_ip=$(kubectl exec -it $sdwan_pod_name ifconfig $wan_interface  | awk '/inet/{print $2}' | cut -f2 -d ":" | awk 'NR==1 {print $1}')
ping_result=$(kubectl exec -it $ovn_pod_name -- ping -c 3 $sdwan_pod_wan_ip)
...
ping_result=$(kubectl exec -it $ovn_pod_name -- ping -c 3 $sdwan_pod_wan_ip 2>&1 || true)
