...
sriov_capable_nodes=$(kubectl get nodes -o json | jq -r '.items[] | select((.status.capacity."openshift.io/intel_sriov_nic"|tonumber)>=2) | .metadata.name')
...
spec:
  ...
  - name: test-pod
    ...
    resources:
      requests:
        openshift.io/intel_sriov_nic: '1'
      limits:
        openshift.io/intel_sriov_nic: '1'
POD
}
...
spec:
  ...
  - name: test-pod
    ...
    resources:
      requests:
        openshift.io/intel_sriov_nic: '2'
      limits:
        openshift.io/intel_sriov_nic: '2'
POD
}
...
for podType in ${POD_TYPE:-single multiple}; do
    ...
    allocated_node_resource=$(kubectl describe node | grep "openshift.io/intel_sriov_nic" | tail -n1 |awk '{print $(NF)}')
