...
function _get_ovn_central_address {
   #host_folder=$test_folder/../../hosting_provider/vagrant/inventory/hosts.ini
    ansible_ifconfig=$(ansible ovn-central[0] -i $test_folder/../../hosting_provider/vagrant/inventory/hosts.ini -m shell -a "ifconfig eth1 |grep \"inet addr\" |awk '{print \$2}' |awk -F: '{print \$2}'")
