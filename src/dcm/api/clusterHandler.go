...
type clusterHandler struct {
    client logicalcloud.ClusterManager
}
...
func (h clusterHandler) createHandler(w http.ResponseWriter, r *http.Request) {
    ...
    ret, err := h.client.Create(project, logicalCloud, clusterProvider, v)
    if err != nil {
...
func (h clusterHandler) getHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if len(name) == 0 {
        ret, err = h.client.GetAll(project, logicalCloud, clusterProvider)
        if err != nil {
            http.Error(w, err.Error(), http.StatusInternalServerError)
            return
        }
    } else {
        ret, err = h.client.Get(project, logicalCloud, clusterProvider, name)
        if err != nil {
            http.Error(w, err.Error(), http.StatusInternalServerError)
            return
        }
...
// Name is required.
    ...
    ret, err := h.client.Update(project, logicalCloud, clusterProvider, name, v)
    if err != nil {
...
func (h clusterHandler) deleteHandler(w http.ResponseWriter, r *http.Request) {
    ...
    err := h.client.Delete(project, logicalCloud, clusterProvider, name)
    if err != nil {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
