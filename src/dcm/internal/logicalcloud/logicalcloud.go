...
import (
        ...
        "github.com/onap/multicloud-k8s/src/dcm/internal/db"
...
type MetaDataList struct {
        ...
        UserData2           string  `json:"userData1"`
}
...
// Cluster contains the parameters needed for the cluster association
type Cluster struct {
...
func (v *LogicalCloudClient) Associate(name string, c Cluster) (Cluster, error) {
    key := LogicalCloudKey {
        ClusterReference: c.ClusterReference,
    }
    //Check if this Logical Cloud exists
    _, err := v.Get(name)
    if err != nil {
