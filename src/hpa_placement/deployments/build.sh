...
component="hpa_placement"

VERSION="0.5.0-SNAPSHOT"
export IMAGE_NAME="nexus3.onap.org:10003/onap/multicloud/k8s/$component"

function _compile_src {
    echo "Compiling source code"
    pushd $k8s_path/src/$component/
    make
    popd
}
...
function _move_bin {
    ...
    rm -f $component *so
    mv $k8s_path/src/$component/$component .
...
function _cleanup {
    ...
    docker-compose kill
    image=$(grep "image.*$component" docker-compose.yml)
    if [[ -n ${image} ]]; then
