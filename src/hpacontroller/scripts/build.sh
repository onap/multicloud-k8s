...
export IMAGE_NAME="nexus3.onap.org:10003/onap/multicloud/k8s/hpa-controller"

function _compile_src {
    echo "Compiling source code"
    pushd $k8s_path/src/hpacontroller/
    make
    popd
}
...
function _move_bin {
    ...
    mv $k8s_path/src/hpacontroller/hpacontroller .
}

function _cleanup {
    echo "Cleaning previous execution"
    docker-compose kill
    image=$(grep "image.*hpacontroller" docker-compose.yml)
    if [[ -n ${image} ]]; then
