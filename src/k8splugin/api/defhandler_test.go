...
func TestRBDefListVersionsHandler(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.label, func(t *testing.T) {
			vh := rbDefinitionHandler{client: testCase.rbDefClient}
			req, err := http.NewRequest("GET", "/v1/rb/definition/testresourcebundle", nil)
			if err != nil {
				t.Fatal(err)
			}
			rr := httptest.NewRecorder()
			hr := http.HandlerFunc(vh.listVersionsHandler)
...
func TestRBDefGetHandler(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.label, func(t *testing.T) {
			vh := rbDefinitionHandler{client: testCase.rbDefClient}
			req, err := http.NewRequest("GET", "/v1/rb/definition/"+testCase.name+"/"+testCase.version, nil)
			if err != nil {
				t.Fatal(err)
			}
