	...
	for _, testCase := range testCases {
		t.Run(testCase.label, func(t *testing.T) {
			...
			if rr.Code == http.StatusOK {
				...
				if reflect.DeepEqual(testCase.expected[0], got[0]) == false &&
					reflect.DeepEqual(testCase.expected[0], got[1]) == false {
					t.Errorf("listHandler returned unexpected body: got %v;"+
						" expected %v", got, testCase.expected)
				}

				if reflect.DeepEqual(testCase.expected[1], got[1]) == false &&
					reflect.DeepEqual(testCase.expected[1], got[0]) == false {
					t.Errorf("listHandler returned unexpected body: got %v;"+
						" expected %v", got, testCase.expected)
				}
			}
