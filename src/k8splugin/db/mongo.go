...
type MongoCollection interface {
	...
	FindOne(ctx context.Context, filter interface{},
		opts ...*options.FindOneOptions) (mongo.Cursor, error)
	Find(ctx context.Context, filter interface{},
		opts ...*options.FindOptions) (mongo.Cursor, error)
}

// MongoStore is an implementation of the MongoClient interface
...
func NewMongoStore(name string) (Store, error) {
	...
	mongoDB := mongoClient.Database(name)
...
func (m *MongoStore) HealthCheck(coll string) error {
	if coll == "" {
		log.Println("Invalid collection name type in options, using healthcheck")
		coll = "healthcheck"
	}
...
func (m *MongoStore) ReadAll(coll, tag string) ([][]byte, error) {
	if coll == "" || tag == "" {
	...
	for cursor.Next(ctx) {
		...
		if err != nil {
			log.Printf("Unable to decode data in Readall")
			continue
		}
		...
		result = append(result, tagData.Lookup(tag).Value)
	}
