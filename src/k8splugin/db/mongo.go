...
func (m *MongoStore) HealthCheck() error {
	...
	if err != nil {
		return pkgerrors.Errorf("Unable to create healtcheck entry in database %s", err.Error())
	}
...
func (m *MongoStore) Delete(coll, key, tag string) error {
	...
	keydata, err := c.FindOneAndUpdate(ctx, filter, update,
		options.FindOneAndUpdate().SetReturnDocument(options.Before)).DecodeBytes()
	if err != nil {
