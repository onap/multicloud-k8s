...
func TestGetHookByEvent(t *testing.T) {
	...
	t.Run("Get pre-install hook", func(t *testing.T) {
		...
		if preinstallList[0].Hook.Name != "preinstall1" {
			t.Fatalf("TestGetHookByEvent error: expect name of 1st preinstall hook is preinstall1, result= %s", preinstallList[0].Hook.Name)
		}
		if preinstallList[1].Hook.Name != "preinstall2" {
			t.Fatalf("TestGetHookByEvent error: expect name of 2nd preinstall hook is preinstall2, result= %s", preinstallList[0].Hook.Name)
		}
	})
