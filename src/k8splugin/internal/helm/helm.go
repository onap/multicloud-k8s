...
// Define hooks that are honored by k8splugin (Helmv3 doesn't have these defined)
...
func (h *TemplateClient) GenerateKubernetesArtifacts(inputPath string, valueFiles []string,
	values []string) ([]KubernetesResourceTemplate, []*release.Hook, error) {
	...
	hookList, m, err := releaseutil.SortManifests(rmap, nil, releaseutil.InstallOrder)
	if err != nil {
		return nil, hookList, err
	}
	for _, k := range m {
