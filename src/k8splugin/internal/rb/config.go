...
// ConfigManager is an interface exposes the resource bundle profile functionality
...
// Get resource entries in the database
...
func (v *ConfigClient) Rollback(rbName, rbVersion, profileName string, rback ConfigRollback) error {
	...
	// Update config to the new latest version
	configNew, _, action, err := cvs.getConfigVersion(rollbackIndex)
	cs := ConfigStore{
		rbName:      rbName,
		rbVersion:   rbVersion,
		profileName: profileName,
	...
	if action == "PUT" || action == "POST" {
		_, err = cs.updateConfig(configNew)
		if err != nil {
