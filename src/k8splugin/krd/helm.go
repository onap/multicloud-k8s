...
// HelmTemplateClient implements the HelmTemplate interface
// It will also be used to maintain any localized state
type HelmTemplateClient struct {
...
// NewHelmTemplateClient returns a new instance of HelmTemplateClient
func NewHelmTemplateClient(k8sversion, namespace, releasename string) *HelmTemplateClient {
	return &HelmTemplateClient{
...
func (h *HelmTemplateClient) processValues(valueFiles []string, values []string) ([]byte, error) {
...
func (h *HelmTemplateClient) mergeValues(dest map[string]interface{}, src map[string]interface{}) map[string]interface{} {
	for k, v := range src {
...
func (h *HelmTemplateClient) ensureDirectory(f string) error {
	base := path.Dir(f)
	_, err := os.Stat(base)
...
func (h *HelmTemplateClient) GenerateKubernetesArtifacts(inputPath string, valueFiles []string, values []string) (map[string][]string, error) {
	...
	for _, m := range tiller.SortByKind(manifestsToRender) {
		...
		mfilePath := strings.Join([]string{outputDir, m.Name}, string(filepath.Separator))
