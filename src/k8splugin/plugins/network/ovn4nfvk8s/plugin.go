...
var ReadNetworkFile = func(path string) (Ovn4NfvK8sFile, error) {
	...
	log.Printf("Reading Network YAML: " + path)
...
func CreateNetwork(data *krd.ResourceData, client kubernetes.Interface) (string, error) {
	...
	log.Printf("CreateNetwork: Ovn4nfvk8s")
...
func GetNetwork(name string, client kubernetes.Interface) (string, error) {
	...
	log.Printf("GetNetwork: Ovn4nfvk8s %s", name)
	return "", nil
}
...
func run(cmdPath string, args ...string) (*bytes.Buffer, *bytes.Buffer, error) {
	...
	//log.Printf("exec: %s %s", cmdPath, strings.Join(args, " "))
	err := cmd.Run()
	if err != nil {
		log.Printf("exec: %s %s => %v", cmdPath, strings.Join(args, " "), err)
	}
...
func createOvnNetwork(name, subnet, gatewayIPMask string) error {
	...
	if err != nil {
		log.Printf("Failed to get logical router port,stderr: %q, error: %v", stderr, err)
		return err
	}
	...
	if err != nil {
		log.Printf("Failed to add logical port to router, stderr: %q, error: %v", stderr, err)
		return err
	}
	...
	if err != nil {
		log.Printf("Failed to create a logical switch %v, stdout: %q, stderr: %q, error: %v", name, stdout, stderr, err)
		return err
	}
	...
	if err != nil {
		log.Printf("Failed to add logical port to switch, stdout: %q, stderr: %q, error: %v", stdout, stderr, err)
		return err
	}
	return nil
...
func deleteOvnNetwork(name string) error {
	...
	if err != nil {
		log.Printf("Failed to delete switch %v, stdout: %q, stderr: %q, error: %v", name, stdout, stderr, err)
		return err
	}

	stdout, stderr, err = RunOVNNbctl(getAuthStr(), "--if-exist", "lrp-del", "rtos-"+name)
	if err != nil {
		log.Printf("Failed to delete router port %v, stdout: %q, stderr: %q, error: %v", name, stdout, stderr, err)
		return err
	}

	stdout, stderr, err = RunOVNNbctl(getAuthStr(), "--if-exist", "lsp-del", "stor-"+name)
	if err != nil {
		log.Printf("Failed to delete switch port %v, stdout: %q, stderr: %q, error: %v", name, stdout, stderr, err)
		return err
	}
	return nil
