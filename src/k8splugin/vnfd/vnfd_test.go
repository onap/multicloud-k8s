...
func TestCreateEntry(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			label: "Create VNF Definition Entry in Database",
			inp: VNFDefinition{
		...
		{
			label:         "Failed Creation of Entry in Database",
			expectedError: "Error Creating Entry",
			mockdb: &mockDB{
				Err: pkgerrors.New("Error Creating Entry"),
			},
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.label, func(t *testing.T) {
			...
			err := testCase.inp.createEntry("vnfd/")
			if err != nil {
				if testCase.expectedError == "" {
					t.Fatalf("CreateEntry returned an unexpected error %s", err)
				}
