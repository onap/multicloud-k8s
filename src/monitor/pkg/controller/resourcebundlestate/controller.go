...
func (r *reconciler) Reconcile(req reconcile.Request) (reconcile.Result, error) {
	...
	// Update the CR with the pod status for all pods with the
	// label selector from the CR
	listOptions := &client.ListOptions{
		Namespace:     rbstate.Namespace,
		LabelSelector: labels.SelectorFromSet(rbstate.Spec.Selector.MatchLabels),
	}

	err = r.updatePods(rbstate, listOptions)
	if err != nil {
		log.Printf("Error adding podstatuses: %v\n", err)
		return reconcile.Result{}, err
	}

	err = r.updateServices(rbstate, listOptions)
	if err != nil {
...
func (r *reconciler) updateServices(rbstate *v1alpha1.ResourceBundleState,
	lopts *client.ListOptions) error {

	// Update the CR with the Services created as well
	serviceList := &corev1.ServiceList{}
	err := r.client.List(context.TODO(), lopts, serviceList)
	if err != nil {
...
func (r *reconciler) updatePods(rbstate *v1alpha1.ResourceBundleState,
	...
	err := r.client.List(context.TODO(), lopts, podList)
	if err != nil {
		log.Printf("Failed to list pods: %v", err)
		return err
	}
