...
func validateWorkloadIfIntentInputs(wif moduleLib.WorkloadIfIntent) error {
	...
	if len(wif.Spec.DefaultIf) > 0 {
		errs = validation.IsValidName(wif.Spec.DefaultIf)
		if len(errs) > 0 {
			return pkgerrors.Errorf("Invalid default interface = [%v], errors: %v", wif.Spec.DefaultIf, errs)
		}
	}
