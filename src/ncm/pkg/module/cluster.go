...
func (v *ClusterClient) ApplyNetworkIntents(provider, name string) error {
	...
	//_, err = ac.AddInstruction(handle, "app", "order", `{"apporder":["network-intents"]}`)
	...
	for _, resource := range resources {
		/*
			...
			orderinstr = orderinstr + `"` + resource.name + `"`
			depinstr = depinstr + `"` + resource.name + `":"go"`
		*/
		orderinstr.Resorder = append(orderinstr.Resorder, resource.name)
		resdep[resource.name] = "go"
		_, err = ac.AddResource(clusterhandle, resource.name, []byte(resource.value))
		if err != nil {
			cleanuperr := ac.DeleteCompositeApp()
			if cleanuperr != nil {
	...
	/*
		_, err = ac.AddInstruction(clusterhandle, "resource", "order", `{"resorder":[`+orderinstr+"]}")
		_, err = ac.AddInstruction(clusterhandle, "resource", "dependency", `{"resdependency":{`+depinstr+"}}")
	*/
