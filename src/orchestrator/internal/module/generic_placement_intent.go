 ...
 */
 ...
 type GenericPlacementIntent struct {
	MetaData IntentMetaDataFields `json:"metadata"`
	Spec IntentSpecDataFields	    `json:"spec"`
}


// IntentMetaDataFields has name, description, userdata1, userdata2
 type IntentMetaDataFields struct {
 ...
 // IntentSpecDataFields has logical-cloud-name
 type IntentSpecDataFields struct {
...
// NewGenericPlacementIntentClient return an instance of GenericPlacementIntentClient which GenericPlacementIntentManager
...
func (c *GenericPlacementIntentClient) CreateGenericPlacementIntent(g GenericPlacementIntent, p string, ca string,
	...
	_, err = NewProjectClient().Get(p)
	if err != nil {
		return GenericPlacementIntent{}, pkgerrors.New("Unable to find the project")
	}

	// check if compositeApp exists
	_, err = NewCompositeAppClient().Get(ca)
	if err != nil {
		return GenericPlacementIntent{}, pkgerrors.New("Unable to find the composite-app")
	}

	// check if compositeApp version exists
	_, err = NewCompositeAppClient().GetVersion(v)
	if err != nil {
		return GenericPlacementIntent{}, pkgerrors.New("Unable to find the version of composite-app")
	}

	metaDataFields := IntentMetaDataFields{
		Name: g.MetaData.Name,
		Description: g.MetaData.Description,
		UserData1: g.MetaData.UserData1,
		UserData2: g.MetaData.UserData2,
	}

	specDataFields := IntentSpecDataFields{
		LogicalCloud: g.Spec.LogicalCloud,
	}

	gPIntent := GenericPlacementIntent{
	...
	err = db.DBconn.Create(c.storeName, gkey, c.tagMetaData, g)
	if err !=nil {
...
func (c *GenericPlacementIntentClient) GetGenericPlacementIntent(i string, p string, ca string, v string) (GenericPlacementIntent, error){
	...
	if result != nil {
		...
		err = db.DBconn.Unmarshal(result, &g)
		if err!=nil {
