...
func createClient(Host string, Port string, Name string) error {
	...
	if string.Contains(config.GetConfiguration().GrpcTLS, "enable") {
		tls = true
	} else {
		tls = false
	}
	...
	if string.Contains(Name, "sync") {
		RPC[Name], err = syncpb.NewSyncClient(conn)
	} else {
		RPC[Name], err = controllerpb.NewControllerClient(conn)
	}
	if err != nil {
