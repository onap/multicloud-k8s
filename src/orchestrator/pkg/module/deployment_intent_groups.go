...
func (c *DeploymentIntentGroupClient) InstantiateDeploymentIntentGroup(di string, p string, ca string, v string) error {
	...
	mc := NewControllerClient()
	ctrl, err := mc.GetController(controllerName)
	if err != nil {
		return pkgerrors.Wrapf(err, "Updatecontext - error finding controller in db: %v", controllerName)
	}
	getConn := rpc.GetRpcConnReq(controllerName, ctrl.Spec.Host, ctrl.Spec.Port)
	rpc.RpcCtl <- getConn
