...
func (mc *MicroserviceClient) CreateMicroservice(m Microservice) (Microservice, error) {
	...
	_, err := mc.GetMicroservice(m.Name)
	if err == nil {
		return Microservice{}, pkgerrors.New("Microservice already exists")
	}

	err = db.DBconn.Create(m.Name, key, mc.tagMeta, m)
	if err != nil {
...
func (mc *MicroserviceClient) GetMicroservice(name string) (Microservice, error) {

	//Construct the composite key to select the entry
	key := MicroserviceKey{
		MicroserviceName: name,
	}
	value, err := db.DBconn.Read(name, key, mc.tagMeta)
	if err != nil {
...
func (mc *MicroserviceClient) DeleteMicroservice(name string) error {
