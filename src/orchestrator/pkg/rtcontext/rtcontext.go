...
type Rtcontext interface {
	...
	RtcCreate() (interface{}, error) // RtcCreate is deprecated, use RtcCreateNew.
	RtcCreateNew(meta interface{}) (interface{}, error)
...
// RtcCreate is RtcCreate deprecated, Use RtcCreateNew
...
//RtcCreateNew is used for saving meta data of appContext into ETCD. Dont use RtcCreate() method, its deprecated
func (rtc *RunTimeContext) RtcCreateNew(meta interface{}) (interface{}, error) {
	cid := fmt.Sprintf("%v", rtc.cid)
	if cid == "" {
		return nil, pkgerrors.Errorf("Error, context not intialized")
	}
	...
	id := strings.SplitN(cid, "/", 4)[2]
	//sc := StoreContext{id: id, meta: meta}
	err := contextdb.Db.Put(cid, id)
	if err != nil {
		return nil, pkgerrors.Errorf("Error creating run time context: %s", err.Error())
	}
	...
	if err != nil {
		return nil, pkgerrors.Errorf("Error saving metadata in run time context: %s", err.Error())
	}
