...
func GetClusterResources(rbData rb.ResourceBundleStatus, qOutput string, qResources []string, resourceList *[]ResourceStatus, cnts map[string]int) (int, error) {
	count := 0
...
func PrepareStatusResult(stateInfo state.StateInfo, apps []string, qInstance, qType, qOutput string, qApps, qClusters, qResources []string) (StatusResult, error) {
	...
	for _, app := range apps {
		...
		if err != nil {
			log.Info(":: No clusters for app ::", log.Fields{"AppName": app})
			continue
		}
