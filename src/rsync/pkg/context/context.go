	...
	for {
		if err := c.IsReachable(); err != nil {
			if !resStateUpdated {
				...
				logutils.Info("waitForclusterReady - updated resources", logutils.Fields{"app": appname})
			}
			// TODO: Add more realistic error checking
			// TODO: Add Incremental wait logic here
			select {
			case <-ch:
				logutils.Info("waitForclusterReady - got an exit message", logutils.Fields{"app": appname})
			...
			case <-time.After(waitTime * time.Second):
				logutils.Info("waitForclusterReady - got a timer message", logutils.Fields{"app": appname})
				break
			}
		} else {
...
func waitForDone(ac appcontext.AppContext) {
	...
	for {
		...
		logutils.Info("Waiting for done watcher running..", logutils.Fields{"status": acStatus.Status})
	}
	return
}

func kickoffRetryWatcher(instca *CompositeAppContext, ac appcontext.AppContext, wg *errgroup.Group) {
...
func getAppContextFlag(ac appcontext.AppContext) (bool, error) {
	...
	if sh == nil {
		...
		logutils.Info("getAppContextFlxg", logutils.Fields{"flag": v, "boolean": v.(bool)})
		if err != nil {
			return false, err
		} else {
			return v.(bool), nil
...
func applyFnComApp(instca *CompositeAppContext, acStatus appcontext.AppContextStatus, f fn, sfn statusfn, breakonError bool) error {
	...
	kickoffRetryWatcher(instca, ac, wg)
