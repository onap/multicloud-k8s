...
func deleteResource(clustername string, resname string, respath string) error {
	...
	err = gp.DeleteByNameAndKind(respath, resname, "default", c)
	if err != nil {
...
// Delete all the apps
func  deleteApps(ac appcontext.AppContext, appsmap []instMap) error {
	fmt.Printf("\n inst app called with : %v \n", appsmap)
	//var chans [5]chan int
	var wg sync.WaitGroup
	var chans = make([]chan int, len(appsmap))
...
func (instca *CompositeAppContext) DeleteComApp(cid interface{}) error {
	ac := appcontext.AppContext{}
	...
	fmt.Println(instca.appsmap)
	err = deleteApps(ac, instca.appsmap)
        if err != nil {
                return err
        }
