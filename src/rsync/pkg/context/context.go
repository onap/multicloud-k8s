...
func waitForClusterReady(ac appcontext.AppContext, c *kubeclient.Client, cluster string) error {
	for {
		if err := c.IsReachable(); err != nil {
                        logutils.Info("Cluster is not reachable::", logutils.Fields{
                                "cluster": cluster,
                        })
			...
			for k := 0; k < len(clusterNames); k++ {
				...
				g.Go(func() error {
					...
					for {
						...
						err = waitForClusterReady(ac, c, cluster)
						if err != nil {
