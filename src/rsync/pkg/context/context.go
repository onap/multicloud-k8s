...
func getInstMap(order string, dependency string, level string) ([]instMap, error) {
        ...
        for i, u := range appso {
                //instmap[i] = instMap{u.(string), appsd[u.(string)].(string), "none", nil, []clusterInfo{}}
...
func createResource(clustername string, respath string) error {
	k8sClient := app.KubernetesClient{}
	err := k8sClient.Init(clustername, "12345")
	if err != nil {
...
*/
	...
	if resval != "" {
		...
		respath := "/vagrant/test/resources/" + resmap.name + ".yaml"
		...
		err = createResource(clustername, respath)
		if err != nil {
			return err
		}
	}
...
func (instca *CompositeAppContext) InstantiateComApp(cid interface{}) error {
	...
	//instca := CompositeAppContext{}
        //instca.appsorder = `{"apporder": ["app2", "app1", "app3"]}`
        //instca.appsdependency = `{"appdependency": {"app1": "wait on app2", "app2": "go", "app3": "wait on app1"}}`
	...
	for j:=0; j<len(instca.appsmap); j++ {
		...
		for k:=0; k<len(clusternames); k++ {
			...
			//clusterh, err := ac.GetClusterHandle(instca.appsmap[j].name, clusternames[k])
			//if err != nil {
		         //       return err
			//}
...
/*
	appnames, err := ac.GetAppnames()
        if err != nil {
                return err
        }
	for _, aname := range appnames {
		clusternames, err := ac.GetClusterNames(aname)
		if err != nil {
			re://gerrit.onap.org/r/c/multicloud/k8s/+/105982turn err
		}
	}
*/
...
func main() {
	...
	ca := CompositeAppContext{}
	ca.cid = "test888888887878"

        //o := `{"apporder": ["app2", "app1", "app3"]}`
        ca.appsorder = `{"apporder": ["app2", "app1", "app3"]}`
        //d := `{"appdependency": {"app1": "wait on app2", "app2": "go", "app3": "wait on app1"}}`
        ca.appsdependency = `{"appdependency": {"app1": "wait on app2", "app2": "go", "app3": "wait on app1"}}`
        r, err := getInstMap(ca.appsorder,ca.appsdependency, "app")
	if err != nil {
		fmt.Printf("\n get instatiation map failed \n")
	}
        fmt.Println(r)
        fmt.Println(len(r))
	ca.appsmap = r

	ca.appsmap[0].cluster =  make([]clusterInfo, 3)
	ca.appsmap[0].cluster[0].name = "cluster1"
        fmt.Println(ca.appsmap)
        ...
        fmt.Println()
        fmt.Println()
	err = instantiateApps(r)
        if err != nil {
		fmt.Printf("\n app instatiation failed: %v\n", err.Error())
        }
	fmt.Printf("\n instatiation apps  completed \n")
        fmt.Println(r)

	ca.appsmap[0].cluster[0].resorder = `{"resorder": ["R3", "R1", "R2"]}`
	ca.appsmap[0].cluster[0].resdependency = `{"resdependency": {"R1": "wait on R3", "R2": "wait on R1", "R3": "go"}}`
        //ro := `{"resorder": ["R3", "R1", "R2"]}`
        //rd := `{"resdependency": {"R1": "wait on R3", "R2": "wait on R1", "R3": "go"}}`
        r, _ = getInstMap(ca.appsmap[0].cluster[0].resorder,ca.appsmap[0].cluster[0].resdependency, "res")
	ca.appsmap[0].cluster[0].ressmap = r
        fmt.Println(r)
        fmt.Println(ca.appsmap[0].cluster[0])
        //time.Sleep(time.Second)
	err = instantiateResources(r)
        if err != nil {
		fmt.Printf("\n app instatiation failed: %v\n", err.Error())
        }
	fmt.Printf("\n instatiation res  completed \n")
}
